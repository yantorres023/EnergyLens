...
class BillTextParser {
  ...
  ParseResult parse(String text) {
    ...
    final unitRates = <String, List<_Hit>>{'single': [], 'day': [], 'night': []};
    ...
    for (final line in lines) {
      ...
      final reg =
          RegExp(r'night|off-?\s?peak|low rate', caseSensitive: false).hasMatch(line)
          ? 'night'
          : RegExp(
      ...
      for (final m in RegExp(
        ...
        final pence = '${int.parse(digits.substring(0, 2))}'
      ...
      if (standing != null && !fields.containsKey(ParsedFieldKey.standingCharge)) {
        fields[ParsedFieldKey.standingCharge] = ParsedField(
          ...
          RegExp(r'£\s*(0\.\d+)\s*(?:per|/|a)\s*day', caseSensitive: false)
              .hasMatch(line)) {
        final pm = RegExp(r'£\s*0\.(\d+)\s*(?:per|/|a)\s*day', caseSensitive: false)
            .firstMatch(line)!;
        final digits = pm.group(1)!.padRight(2, '0');
        fields[ParsedFieldKey.standingCharge] = ParsedField(
          key: ParsedFieldKey.standingCharge,
          value: '${int.parse(digits.substring(0, 2))}'
              '${digits.length > 2 ? '.${digits.substring(2)}' : ''}',
          sourceSnippet: line,
          confidence: ParseConfidence.high,
        );
        ...
        final m = RegExp('(?:x|@|at)\\s*$_num\\s*p\\b', caseSensitive: false)
            .firstMatch(line);
        if (m != null) {
          fields[ParsedFieldKey.standingCharge] = ParsedField(
            key: ParsedFieldKey.standingCharge,
      ...
      if (RegExp(r'total', caseSensitive: false).hasMatch(line) &&
          !RegExp(r'balance|paid|payment|annual|kwh', caseSensitive: false)
              .hasMatch(line)) {
        final money = RegExp(r'(-?£\s*-?[\d,]+\.\d{2}(?:\s*CR)?)', caseSensitive: false)
            .allMatches(line)
            .toList();
        if (money.isNotEmpty) {
          final pence = parsePoundsToPence(money.last.group(1)!.replaceAll(' ', ''));
          ...
          if (pence != null &&
            ...
            fields[ParsedFieldKey.statedTotal] = ParsedField(
              ...
              value: '$pence',
              sourceSnippet: line,
              confidence: isElectric ? ParseConfidence.high : ParseConfidence.low,
            );
          }
        }
      }
      ...
      if (!fields.containsKey(ParsedFieldKey.readingType)) {
        ReadingType? rt;
        if (RegExp(r'\bestimated?\b|\(e\)', caseSensitive: false).hasMatch(line) &&
        ...
        } else if (RegExp(r'\bactual\b|\(a\)', caseSensitive: false).hasMatch(line) &&
            RegExp(r'read', caseSensitive: false).hasMatch(line)) {
          rt = ReadingType.actual;
        } else if (RegExp(r'customer read|your read|\(c\)', caseSensitive: false)
            .hasMatch(line)) {
    ...
    void pick(String reg, ParsedFieldKey rateKey, ParsedFieldKey kwhKey) {
      ...
      if (rates.isNotEmpty) {
        ...
        fields[rateKey] = ParsedField(
          ...
          confidence: distinct.length == 1 ? ParseConfidence.high : ParseConfidence.low,
        );
      }
      final uses = kwh[reg]!;
      if (uses.isNotEmpty) {
        ...
        final withRate = uses.where(
          (h) => RegExp(r'(?:x|@|at)\s*[\d.]+\s*p', caseSensitive: false).hasMatch(h.line),
        );
        ...
        fields[kwhKey] = ParsedField(
          ...
          confidence: withRate.isNotEmpty ? ParseConfidence.high : ParseConfidence.low,
        );
      }
    }
  ...
  void _parsePeriod(String text, Map<ParsedFieldKey, ParsedField> fields) {
    ...
    if (named != null) {
      ...
      final startMonth = _months[named.group(2)!.toLowerCase().substring(0, 3)]!;
